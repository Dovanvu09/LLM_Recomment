...
This repository uses Groq to create a movie recommender system and compares the recommender's performance across four prompt templates.

To run the code in this repository using Groq, first pip install -r requirements.txt, then get an API Key from Groq and include it in a python file called key.py. The file only needs one line stating:
```python
...
The recommender's performance was compared using the MovieLens 100k dataset from the below paper. It can be found here: MovieLens Dataset
